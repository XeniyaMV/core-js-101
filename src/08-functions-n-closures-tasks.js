...
function getPolynom(...args) {
  const coefs = Array.from(args).reverse();
  return (x) => coefs.reduce((polynom, item, i) => polynom + item * (x ** i), 0);
}
...
function retry(func, attempts) {
  ...
  return function retryer(...args) {
    try {
      ...
      if (counter < attempts) retryer(...args);
    }
    return value;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const strArg = args.map((item) => JSON.stringify(item));
